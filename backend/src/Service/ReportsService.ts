import { ReportRepository } from "../Repository/ReportsRepository"
import { ReportResult } from "../Schema/ReportsSchema"

export class ReportService {
    constructor(private reportsRepository: ReportRepository) {}

    async getReportService(userId: number): Promise<ReportResult> {
        const now = new Date()
        ...
        const categoryMap: Record<string, { name: string, color: string, logo: string, total: number }> = {}
        ...
        const lastMonthValue = await this.reportsRepository.getLastMonthTotal(
            userId,
            startOfLastMonth,
            endOfLastMonth
        )
